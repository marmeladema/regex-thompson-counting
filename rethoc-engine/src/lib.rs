...
enum RegexHirNode {
    ...
    Assert(AssertKind),
    /// Dense bounded single-atom repeat.  Emits `max` consuming states:
    /// - states `0 .. min-1`: plain consuming (out only)
    /// - states `min-1 .. max-2`: consuming with out_exit (fused branch)
    /// - state `max-1`: plain consuming (out → exit)
    ///
    /// Pops the atom fragment from the stack and re-uses its state kind.
    /// `min >= 1` and `max > min`.
    DenseBoundedAtom {
        min: usize,
        max: usize,
...
pub struct Regex {
    ...
    /// (`Anchor (Gap Anchor)* Gap?`).  The plan contains separately
    /// compiled anchor programs and gap constraints.  When present,
    /// default matcher dispatch prefers the bounded-gap specialisation.
    pub(crate) bounded_gap_plan: Option<bounded_gap::BoundedGapPlan>,
}
...
impl Regex {
    ...
    pub(crate) fn into_anchor_program(self) -> bounded_gap::AnchorProgram {
        use bounded_gap::{AnchorEngine, AnchorProgram};
        let engine = if self.dfa_eligible {
            AnchorEngine::Dfa
        } else {
            AnchorEngine::Nfa
        };
    ...
    pub fn info(&self) -> RegexInfo {
        ...
        RegexInfo {
            ...
            execution: ExecutionInfo {
                ...
                specialization: if self.bounded_gap_plan.is_some() {
                    Some("BoundedGap".to_string())
                } else {
                    None
                },
            },
...
impl RegexBuilder {
    ...
    fn try_unroll(
        ...
        if max != usize::MAX {
            // Dense single-atom path: Atom{min,max} → max consuming states
            // with fused exit branches, no Split overhead.
            if body_nfa == 1 && Self::is_single_atom(sub) {
                let cost = max;
                if cost <= limit {
                    self.hir2postfix(sub)?;
                    self.postfix
                        .push(RegexHirNode::DenseBoundedAtom { min, max });
                    return Ok(true);
                }
            }

            // Generic bounded non-fixed: X{min,max}
            //   → X·…·X · (X·(X·(…·X?)?)?)?
    ...
    fn next_fragment(&mut self, node: RegexHirNode) -> Fragment {
        match node {
            ...
            RegexHirNode::DenseBoundedAtom { min, max } => {
                // Dense bounded single-atom repeat: pop the atom fragment
                // (a single consuming state) and emit `max` consuming states.
                //
                // For `Atom{min,max}` where min >= 1 and max > min:
                //   states 0 .. min-1:   plain consuming (out only)
                //   states min-1 .. max-2: consuming with out_exit
                //   state max-1:         plain consuming (out → exit)
                //
                // Example: a{1,3} → B(exit) B(exit) B
                // Example: a{3,5} → B B B(exit) B(exit) B
                debug_assert!(min >= 1);
                debug_assert!(max > min);
                let atom_frag = self.frags.pop().unwrap();
                // Read the atom's state to clone it for each position.
                let atom_state = self.states.as_slice()[atom_frag.start];

                // Build `max` states in a chain, right to left.
                // Reuse the original atom state as the last in the chain
                // (avoids leaving an orphaned dead state in the NFA array).
                let last = atom_frag.start;
                let mut prev = last;

                // States (max-2) down to 0:
                for pos in (0..max - 1).rev() {
                    // Positions 0..min-1 are mandatory (no exit).
                    // Positions min-1..max-2 get out_exit.
                    let has_exit = pos >= min - 1;
                    let s = self.state(atom_state);
                    // Wire out → prev (next state in chain).
                    *patch_slot_mut(&mut self.states.as_mut_slice()[s], PatchSlot::Out) = prev;
                    if has_exit {
                        // out_exit will be patched to the fragment exit.
                        // Leave it as NONE for now — it's a dangling out.
                    }
                    prev = s;
                }

                // Collect dangling outs:
                // - last state's Out (the final exit of the chain)
                // - each state with has_exit: OutExit
                let mut outs = vec![PatchRef {
                    state: last,
                    slot: PatchSlot::Out,
                }];
                // Walk the chain to find all states with exit edges.
                // The chain is: prev → ... → last.
                // States at positions min-1..max-2 have out_exit = NONE (dangling).
                let mut cursor = prev;
                for pos in 0..max - 1 {
                    if pos >= min - 1 {
                        outs.push(PatchRef {
...
impl MatcherMemory {
    ...
    pub fn matcher<'a>(&'a mut self, regex: &'a Regex) -> AnyMatcher<'a> {
        ...
        if regex.dfa_eligible {
            ...
            cache.prepare(&mut self.dfa_memory, &regex.nfa);
            ...
            cache.prepare(&mut self.dfa_memory, &regex.nfa, analysis);
...
mod tests {
    ...
    fn test_tier1(pattern: &str, re: &Regex, input: &str, expected: bool, unroll: usize) {
        ...
        cache.prepare(&mut memory, &re.nfa);
    ...
    fn test_tier2(pattern: &str, re: &Regex, input: &str, expected: bool, unroll: usize) {
        ...
        cache.prepare(&mut memory, &re.nfa, analysis);
    ...
    match_tests! {
        ...
        // Bug 41: stale deferred assert effects surviving the fast path.
        ...
        // c0 (c{4,10}) breaks, \b emitted as PendingEffect Match atom,
        ...
        // PATH 15a: deferred assert PendingEffect at EOI that FAILS.
        // Counter breaks, tail deposits \B as PendingEffect Match atom.
